// Variant A — "Current": pixel-close replica of index.html (the live Marathons
// page), rendered inside the lab root as the baseline for comparison.
(function () {
  const css = document.createElement('style');
  css.textContent = `
    .v-a .lede-line, .v-a .status-line {
      font-size: 0.8125rem;
      color: var(--muted);
      margin-bottom: 2rem;
    }
  ...
  function render(root, data, theme) {
    root.innerHTML = `
      <main>
        <p class="lede-line">marathon build analysis</p>
    ...
    // --- status + tables (from index.html's load()) ---
    ...
    root.querySelector('.body-builds').innerHTML = rowsBuilds.join('');
    root.querySelector('.body-peaks').innerHTML  = rowsPeaks.join('');

    // --- chart (from index.html's loadChart()) ---
    ...
    function makeSeries(mode) {
      return wSorted.map((marathon, i) => {
        ...
        if (mode === 'fastest') {
          ...
          color        = highlighted ? gradientColor(i, 5, theme.isDark) : theme.baseline;
    ...
    chart.setOption({
      animation: false,
      grid: { top: 16, right: 155, bottom: 24, left: 52 },
      legend: {
        type: 'scroll',
        orient: 'vertical',
        right: 0,
  ...
  LAB.register({
    id: 'a',
    name: 'Current',
    blurb: 'Baseline — today’s Marathons page, unchanged, for side-by-side comparison.',
    render,
  });
})();
