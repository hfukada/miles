...
- `miles/plan_api.py` — `/api/plan`, `/api/plan-adherence`, `/api/plan-progression`, `/api/plan-retrospective`, `/api/plan-readiness` (goal-vs-fitness trend, current-block-vs-reference-build comparison aligned at the same weeks-out cutoff, session execution), versions/diff endpoints backing plan.html; plus the plan-independent `/api/today` (actual-only today/week-so-far/vs-last-week) and `/api/progression?weeks=N` (rolling-window easy/workout trends, no target bands)
...
- `miles/static/` — vanilla JS, no framework. `plan.html` is the landing page (`/` redirects here) — three tabs (Today · Trends · Plan, hash-routed, `TAB_CHARTS` per-tab resize registries since charts built inside a hidden panel measure zero-size) rendered in every state, backed by plan-independent endpoints (`/api/today`, `/api/progression`) so the page never goes fully empty. Today: for an active plan, the session card/week-at-a-glance/ramp/vs-last-week trio, cut off at last_sync_at; otherwise an actual-only dashboard (synced runs linked to activity detail, a day-chip week strip, the same vs-last-week paired bars) with a muted link to the Plan tab. Trends is identical in every state: easy-HR/pace and workout-pace progression (target bands only when an active plan supplies them via `/api/plan-progression`; a plan-independent rolling window via `/api/progression` otherwise), plus a recent-weekly-mileage chart and a one-line current-fitness stat, both plan-independent. Plan: active plan unchanged (instruction note, Readiness — goal-vs-fitness, current-block-vs-reference-build with athlete-flippable reference chips, session execution — planned-vs-actual weekly chart with adherence bands + flag shading, slot-color-coded weekly calendar with clickable day-detail popovers and visible week notes, version history with diffs); a completed-only plan gets a "Retrospective" marker plus the same calendar/chart/versions pointed at its final version; no plan at all gets an instruction card. A multi-session date (plan_days seq > 1 — a double run, or a run plus a strength day) renders every session everywhere (calendar chip, popover, Today card/week-strip), never just the last row; below 640px the calendar collapses from its grid to stacked full-width week cards, no horizontal scrolling. `races.html`: Overview tab (PB stat strip, race-paces scatter with standing-PR labels, all-races archive) plus a per-distance tab (5K/10K/Half/Marathon/50K/Other), each with a stat strip (PR · Latest · Races · the two nearest sub-X barriers the PR has cracked), 3-mode chart, and a distance-aware sortable table (LR columns on long distances only, distance-relevant pace claims). `builds.html` is the canonical builds index (race result Time column, casual efforts muted; `#build-{date}` deep-links flash the row). `build.html#{race-date}` is the drill-down (stat strip, weekly calendar, lap panel, workout-group comparisons; falls back to a fixed distance-bucket window for races without a detected build). `compare.html` is a build-vs-build workbench (max 4, per distance-bucket). `training.html` is fitness + current load: stat strip (Fitness follows the chart's distance toggle), per-distance Fitness chart with hover/click-to-pin evidence (markers encode basis: solid = race-confirmed, ring = workout-anchored, hollow = training floor), an Efficiency HR-vs-pace section (see below), and the full weekly-mileage chart with period/build bands. `years.html` is year-over-year volume (Cumulative default; multi-select year chips — tiles and the races table follow the most recently selected year). `workbook.html` (header ⌗ icon, not a nav link) collects one-off Claude-authored analysis pages uploaded via `/api/workbooks` (title + upload date). The athlete's reference-build choice persists in localStorage as `miles.refBuild.{bucket}` (race date; PR-build fallback) and is shared by plan/builds/build pages. `design-lab.html` + `static/lab/*.js` are a kept-around design playground, not a shipped page. `theme.css` (tokens + shared table/tab/nav classes) and `nav.js` (header nav: Plan · Activities · Races · History ▾ (Builds/Compare/Training/Years, click-toggle dropdown) + Workbook icon + Sync; `chartTheme()` for ECharts) are shared by every page; `charts.js` holds shared `fmt`/color/`staggerEndLabels`/`sparklineSVG`/`makeSortable` helpers
